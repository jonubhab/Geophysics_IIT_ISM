                ...
                for m in mov:
                    for date in datelist:
                        ...
                        for comp in comps:
                            sacfound[comp] = False
                        for comp in comps:
                            fn = os.path.join("STACKS", "%02i" % filter.ref, "%03i_DAYS"%m, comp,"%s_%s" % (netsta1.replace('.', '_'), netsta2.replace('.', '_')),date+".MSEED")
                            if os.path.isfile(fn):
                                traces[comp] = read(fn, format="MSEED")[0]
                                data[comp]=traces[comp].data
                                sacfound[comp] = True
                        if any(sacfound.values()):
                            print(date)
                            for comp in comps:
                            ...
                            for comp_out, data_out in output_data.items():
                                if reliable[comp_out]:
                                    out_dir = os.path.join("STACKS", "%02i" % filter.ref, "%03i_DAYS"%m, comp_out,"%s_%s" % (netsta1.replace('.', '_'), netsta2.replace('.', '_')))
                                    os.makedirs(out_dir, exist_ok=True)

                                    tr_out = traces[comp_out if comp_out in traces else next(c for c in comps if c in traces)].copy()
                                    tr_out.data = data_out

                                    tr_out.stats.channel = comp_out

                                    tr_out.write(os.path.join(out_dir, "%s.MSEED" % date), format="MSEED")
