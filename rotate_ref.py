        ...
        for job in jobs:
            ...
            for filter in filters:
                ...
                for comp in comps:
                    sacfound[comp] = False
                for comp in comps:
                    fn = os.path.join("STACKS", "%02i" % filter.ref, "REF", comp,"%s_%s.MSEED" % (netsta1.replace('.', '_'), netsta2.replace('.', '_')))
                ...
                for comp_out, data_out in output_data.items():
                    if reliable[comp_out]:
                        out_dir = os.path.join("STACKS", "%02i" % filter.ref,"REF",comp_out)
                        ...
                        tr_out.write(os.path.join(out_dir, "%s_%s.MSEED" % (netsta1.replace('.', '_'), netsta2.replace('.', '_'))), format="MSEED")
